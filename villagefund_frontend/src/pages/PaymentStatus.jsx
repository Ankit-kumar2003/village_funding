      ...
      <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 text-center border border-gray-100 dark:border-gray-700 transition-all duration-300">
        ...
        {checking ? (
          <div className="py-8">
            ...
            <h2 className="text-2xl font-bold font-heading text-text dark:text-white mb-3">{t('paymentVerifying')}</h2>
            ...
            {attempts > 1 && (
              <span className="inline-block mt-4 text-xs font-semibold px-3 py-1 bg-orange-50 text-orange-600 dark:bg-orange-950 dark:text-orange-300 rounded-full animate-pulse">
                {t('paymentAttempt')} {attempts} {t('paymentAttemptOf')} 6
              </span>
            )}
          </div>
        ) : (
          <>
            {paymentStatus === 'APPROVED' ? (
              <>
                ...
                <h2 className="text-3xl font-bold font-heading text-text dark:text-white mb-4">{t('paymentSuccess')}</h2>
                <p className="text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
                  {t('paymentSuccessDesc')} <strong className="text-primary font-bold">₹{amount}</strong> {t('paymentSuccessDescSuffix')}
                </p>
                <div className="bg-gray-50 dark:bg-gray-950 p-4 rounded-xl text-sm text-gray-500 dark:text-gray-400 mb-8 border border-gray-100 dark:border-gray-800 break-all select-all font-mono">
                  <strong>{t('paymentOrderId')}:</strong> {orderId}
                </div>
              </>
              ...
              <>
                ...
                <h2 className="text-3xl font-bold font-heading text-text dark:text-white mb-4">{t('paymentPending')}</h2>
                <p className="text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
              ...
              <>
                ...
                <h2 className="text-3xl font-bold font-heading text-text dark:text-white mb-4">{t('paymentFailed')}</h2>
                <p className="text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
                  {t('paymentFailedDesc')}
                </p>
              </>
